...
class SpreadMonitor:
    ...
    def calculate_spread(self) -> tuple[Optional[float], Optional[float]]:
        ...
        Returns:
            (spread_try, spread_percent) or (None, None) if data incomplete
        """
        if not all([self.binance_ask, self.btcturk_bid]):
            return None, None
        
        # Spread = BtcTurk bid - Binance ask (arbitrage opportunity if positive)
        spread_try = self.btcturk_bid - self.binance_ask
        spread_percent = (spread_try / self.binance_ask) * 100
        
        return spread_try, spread_percent
    ...
    def log_spread(self):
        ...
        if spread_try is not None and spread_percent is not None:
            spread_try_str = f"{spread_try:+.2f}"
        ...
        log_line = (
            ...
            f"({btcturk_lat_str}) | "
            f"{spread_color}Spread:{Colors.ENDC} {spread_try_str} TRY ({spread_pct_str})"
        )
